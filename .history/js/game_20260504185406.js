...
function drawBgImage(key, scrollX) {
  ...
  if (key.startsWith('cocina') && scrollX === undefined) {
    const s = Math.min(W / bg.w, H / bg.h);
    if (s < 1) {
      bg.w = Math.floor(bg.w * s);
      bg.h = Math.floor(bg.h * s);
      bg.x = Math.floor((W - bg.w) / 2);
      bg.y = Math.floor((H - bg.h) / 2);
    }
  }
...
function makeGrandmaPickupScene() {
  const door = new Hotspot({
    x: 690,
    y: GROUND,
    label: 'Llamar a la puerta',
    hitboxes: [
      { x: 930, y: 760, w: 240, h: 230 },
      { x: 1005, y: 820, w: 120, h: 115 },
    ],
  });
  const grandma = new Hotspot({
    x: 840,
    y: GROUND,
    label: 'Recoger a mamá',
    hitboxes: [
      { x: 1030, y: 500, w: 300, h: 450 },
      { x: 1140, y: 520, w: 210, h: 380 },
    ],
  });
  return {
